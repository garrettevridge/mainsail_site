...
const CHUM_SYSTEMS: [RegExp, string][] = [
  [/yukon.*summer|summer.*chum/i, "Yukon summer chum"],
  [/yukon.*fall|fall.*chum/i, "Yukon fall chum"],
  [/kuskokwim/i, "Kuskokwim chum"],
];
...
export default function ChumSlide() {
  ...
  const subsChum = useMemo(() => {
    if (!subs) return null;
    const rows = subs.filter((r) => r.species === "chum" && r.harvest_count != null);
    if (rows.length === 0) return null;
    const used = rows.reduce((a, b) => (b.year > a.year ? b : a));
    return { value: used.harvest_count!, year: used.year };
  }, [subs]);
  ...
  const rivers = useMemo(() => {
    ...
    return CHUM_SYSTEMS.map(([re, label]) => {
      ...
      const value = Math.max(...rows.filter((r) => r.year === latest).map((r) => r.actual_count!));
      return { label, value, year: latest };
    }).filter((x): x is NonNullable<typeof x> => x !== null);
  }, [esc]);
  const riverTotal = rivers.reduce((a, r) => a + r.value, 0);

  const escTotal = subsChum?.value ?? 0;
  ...
  return (
    <div className="sm-slide">
      ...
      {/* THE ALASKA SHARE vs SUBSISTENCE — two area-proportional squares */}
      <div className="sm-block">
        <div className="sm-block-label">The Western Alaska share, against the harvest at home{bycatchLatest ? ` · ${bycatchLatest.year}` : ""}</div>
        {alaskaBycatch != null && subsChum ? (
          <>
            <div className="sm-squares">
              <figure className="sm-square">
                <div className="sm-square-box" style={{ width: squarePx(alaskaBycatch, 200), height: squarePx(alaskaBycatch, 200), background: ACCENT }} />
                <figcaption>
              ...
              <figure className="sm-square">
                ...
                <figcaption>
                  <span className="sm-square-val">{k(subsChum.value)}</span>
                  <span className="sm-square-lbl">Chum taken for subsistence statewide</span>
                  <span className="sm-square-sub">ADF&amp;G / NPAFC · {subsChum.year}</span>
                </figcaption>
              </figure>
            </div>
            <div className="sm-magbar-cap" style={{ fontSize: 13, color: "var(--ink-2)" }}>
              The Western Alaska / Yukon portion of the chum bycatch is about{" "}
              <b style={{ color: "var(--ink)" }}>{escTotal > 0 ? Math.round((alaskaBycatch / escTotal) * 100) : 0}%</b> of the chum that Alaskans take for subsistence in a year. Most subsistence chum is harvested in the same Western Alaska and Yukon–Kuskokwim communities whose runs have collapsed.
            </div>
            <div className="sm-context">
              The genetics resolve the region, not the river. Western Alaska and Up/Mid Yukon are reported as separate groups — the most depleted Canadian-border stocks are about <b>{(breakdown.find((r) => r.region === "Up/Mid Yukon")?.mean_pct ?? 0).toFixed(0)}%</b> of the bycatch.
            </div>
            <div className="sm-context">
              In the rivers the picture is stark. 2024 Yukon fall chum passed Pilot Station at about <b>124,000</b> against a <b>300,000</b> escapement goal — short of the U.S.–Canada treaty floor; Kuskokwim weir counts (Kogrukluk, George) have run <b>69–81%</b> below their 2000–2019 averages.{" "}
              <a href="https://www.adfg.alaska.gov/static/applications/dcfnewsrelease/1652059774.pdf" target="_blank" rel="noreferrer">ADF&amp;G</a>
            </div>
          </>
        ) : (
          <div className="sm-chart-body placeholder short">Loading…</div>
      ...
      <div className="sm-block">
        ...
        {alaskaBycatch != null && rivers.length > 0 ? (
          <>
            ...
            <div className="sm-magbar-cap" style={{ fontSize: 13, color: "var(--ink-2)" }}>
              The Western Alaska / Yukon-origin chum bycatch is about{" "}
              <b style={{ color: "var(--ink)" }}>{riverTotal > 0 ? Math.round((alaskaBycatch / riverTotal) * 100) : 0}%</b> of the counted escapement into those same systems. Genetics resolve the region, not the river — the bycatch cannot be assigned to one run.
            </div>
          </>
        ) : (
          <div className="sm-chart-body placeholder short">
            River escapement graphic — wired to chum escapement, pending ingest (see methodology / INGEST_SPECS P6). Until those counts publish, the river shortfalls are summarized above.
          </div>
        )}
      </div>
      ...
      <div className="sm-method">
        ...
        <p>
          <b>River escapement.</b> The river graphic reads counted chum escapement (Yukon summer and fall chum, Kuskokwim) from the same manifest table the Chinook slide uses; chum rows are not yet published, so it shows a placeholder. The in-river figures cited above — 2024 Yukon fall chum ≈ 124,000 against a 300,000 goal, and Kuskokwim weir counts 69–81% below their 2000–2019 averages — are ADF&amp;G in-season reports, shown as cited context, not drawn from the manifest.
        </p>
      </div>
    </div>
  );
